...
def test_chat_answers_and_records_usage(client):
    ...
    response = client.post(f"/chat/{conv_id}", json={"question": "안녕하세요"})
    assert response.status_code == 200
    assert response.json() == {"answer": "가짜 답변입니다", "model": "claude-opus-5"}

    history = client.get(f"/conversations/{conv_id}/messages").json()["messages"]
...
def test_chat_sends_previous_messages_back_to_claude(client):
    ...
    conv_id = new_conversation(client)
    client.post(f"/chat/{conv_id}", json={"question": "내 이름은 지훈이야"})
    client.post(f"/chat/{conv_id}", json={"question": "내 이름 뭐라고 했지?"})
...
def test_chat_on_missing_conversation_is_404(client):
    response = client.post("/chat/99999", json={"question": "안녕"})
    assert response.status_code == 404


def test_chat_rejects_empty_question(client):
    conv_id = new_conversation(client)
    assert client.post(f"/chat/{conv_id}", json={"question": ""}).status_code == 422
